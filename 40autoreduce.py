...
if args.createdatabase:
    database.create("qhy_database.pkl",overwrite=True)
    database.update("qhy_database.pkl", Path("/data/wst/u/wst/PIMA3/rawdata"))
    exit()
if args.updatedatabase:
    database.update("qhy_database.pkl", Path("/data/wst/u/wst/PIMA3/rawdata"))
    exit()
if args.showdatabase:
    database.show("qhy_database.pkl")
    exit()

if args.showobjects:
    database.show_observed_objects(
        "qhy_database.pkl", args.object, args.filter, args.date
    )
    exit()

if args.obsdates:
    database.get_observed_dates(
        "qhy_database.pkl", args.object, args.filter, float(args.exptime)
    )
    exit()
...
_MASTERPATH = _CWD.joinpath("master")
_ISMASTERBIAS = False
_ISMASTERDARK = False
_ISMASTERFLAT = False
...
def create_masterflat(flatdate):
    ...
    maskfiles = create_flat_star_mask(files, k=3)
...
def main():
    ...
    try:
        database.update("qhy_database.pkl", _ARCHIVEPATH)
    except:
        raise FileNotFoundError("no database found. Use --createdatabase")
    database.sort("qhy_database.pkl", ["date", "object"], [True, True])
    ...
    for _DATE in _DATELIST:
        try:
            Path.mkdir(_CWD.joinpath(f"{_OBJECT.upper()}_{_FILTER}_reduced"))
        except:
            pass
        ...
        objectfiles = database.select_lights(
            "qhy_database.pkl", _OBJECT, _FILTER, _DATE, _EXPTIME
        )
        ...
        if "b" in args.steps:
            if not _ISMASTERBIAS:
                biasfiles = database.select_bias("qhy_database.pkl")
                if len(biasfiles) == 0:
                    _BIASCORR = False
                else:
                    _BIASCORR = True
        ...
        if "d" in args.steps:
            if not _ISMASTERDARK:
                darkfiles = database.select_darks("qhy_database.pkl", _EXPTIME)
                if len(darkfiles) == 0:
                    _DARKCORR = False
                else:
                    _DARKCORR = True
        ...
        if "f" in args.steps:
            ...
            flatfiles, flatdate = database.select_flats(
                "qhy_database.pkl", _FILTER, _DATE
            )
        ...
        copy_files(get_filepaths(_TMPPATH,f"{prefix}*.fits"), f"{_OBJECT.upper()}_{_FILTER}_reduced/")
